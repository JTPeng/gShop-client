...
<script>
  ...
  import {reqLoginSms,reqPwdLogin,reqSendCode} from '../../api'
  export default {
    data(){
      return{
        loginWay:true, //登录的方法
    ...
    computed:{
      rightPhone(){
        return /^1\d{10}$/.test(this.phone) //校验手机号码的合法性
      }
    },
    methods:{
      //异步获取短信验证码
      async getCode(){
        //this.computeTime不等于0 才执行
        if (!this.computeTime){
          //启动计时器
          this.computeTime = 30 //初始化计时器秒数
